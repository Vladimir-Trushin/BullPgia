...
public:
  TestCase(const string &name = "", ostream &output = cerr) : name(name),
                                                              output(output),
                                                              passed(0), failed(0),
                                                              start_time(clock())
  {
  ...
  ostream &print(ostream &out, bool show_grade = true) const
  {
    ...
    out << endl
        << "*** CPU time: " << elapsed_seconds << " sec.  Right: " << passed << ".  Wrong: " << failed << ". ";
    if (show_grade)
      out << "Grade: " << grade() << ". ";
    out << "***" << endl;
    return out;
  ...
  TestCase &check_ok(const TFUNC actual_func, const string &context)
  {
    try
    {
  ...
  template <typename TFUNC>
  TestCase &check_throws(const TFUNC actual_func, const string &context)
  {
    try
    {
      actual_func();
  ...
  template <typename TFUNC, typename TVAL>
  TestCase &check_equal(const TFUNC actual_func, const TVAL &expected_value, const string &context)
  {
    try
    {
...
private:
  ...
  template <typename TFUNC, typename TVAL>
  TVAL get_actual_value(TFUNC actual_func, const string &context)
  {
    try
    {
